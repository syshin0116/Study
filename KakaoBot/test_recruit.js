// 테스트 스크립트 - 채용 정보 크롤링 함수 테스트
const axios = require('axios');
const cheerio = require('cheerio');
const SEARCH_API_KEY = "ENWZFaEmnNFcWRpGVSDf1cAT";
...
// OKKY IT 이벤트 정보 크롤링 테스트
function testOkkyItEvents() {
    console.log("=== OKKY IT 이벤트 테스트 시작 ===");
    const currentDate = getCurrentDateTime().split(" ")[0];
    return getOkkyItEvents(currentDate)
        .then(result => {
            console.log(result);
        ...
        .catch(error => {
            ...
            console.log("=== OKKY IT 이벤트 테스트 종료 ===");
            return "테스트 오류: " + error.message;
        });
}

// 잡코리아 개발·데이터 직종별 일자리 수 크롤링 테스트
function testJobKoreaDevData() {
    console.log("=== 잡코리아 개발·데이터 직종별 일자리 수 테스트 시작 ===");
    const currentDate = getCurrentDateTime().split(" ")[0];
    return getJobKoreaDevData(currentDate)
        .then(result => {
            console.log(result);
...
function testJobKoreaItJobs() {
    console.log("=== 잡코리아 IT 직종 일자리 리스트 테스트 시작 ===");
    const currentDate = getCurrentDateTime().split(" ")[0];
    return getJobKoreaItJobs(currentDate)
        .then(result => {
            console.log(result);
...
// ITWorld 뉴스 크롤링 테스트
function testItworldNews() {
    console.log("=== ITWorld 뉴스 테스트 시작 ===");
    const currentDate = getCurrentDateTime().split(" ")[0];
    return getItworldNews(currentDate)
        .then(result => {
            console.log(result);
...
// 전체 채용 정보 API 테스트
async function testAllRecruitApis() {
    console.log("=== 전체 채용 정보 API 테스트 시작 ===");
    testReplier.clearReplies();
    const currentDate = getCurrentDateTime().split(" ")[0];

    try {
        await callRecruitApis("테스트룸", "테스터", testReplier);
        console.log("=== 전체 채용 정보 API 테스트 종료 ===");
        console.log("총 응답 수: " + testReplier.replies.length);
        return testReplier.replies;
    } catch (error) {
        console.error("테스트 오류:", error);
        console.log("=== 전체 채용 정보 API 테스트 종료 ===");
        return ["테스트 오류: " + error.message];
 ...
 * @returns {Promise<string>} 포맷팅된 이벤트 정보
 */
 ...
 * @param {string} currentDate - 현재 날짜 (YYYY-MM-DD)
 * @returns {Promise<string>} 포맷팅된 직종별 일자리 정보
 */
 ...
 * @param {string} currentDate - 현재 날짜 (YYYY-MM-DD)
 * @returns {Promise<string>} 포맷팅된 IT 직종 일자리 정보
 */
 ...
 * ITWorld 뉴스 크롤링
 * @param {string} currentDate - 현재 날짜 (YYYY-MM-DD)
 * @returns {Promise<string>} 포맷팅된 ITWorld 뉴스 정보
 */
    ...
    try {
        ...
        for (const { keyword, url } of urls) {
            try {
                ...
                console.log(`${keyword} URL 처리 중 오류: ${urlError.message}`);
                newsResults.push({ keyword, items: [] });
            }
        }
...
async function runTests() {
    console.log("=== 채용 정보 크롤링 테스트 시작 ===");
    await testOkkyItEvents();
    await testJobKoreaDevData();
    await testJobKoreaItJobs();
    await testItworldNews();
    console.log("=== 채용 정보 크롤링 테스트 종료 ===");
}
