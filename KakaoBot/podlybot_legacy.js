...
function getResponseFromApi(url, room, sender, msg) {
    try {
        ...
        const searchJsonResponse = JSON.parse(responseText);

        // 검색 결과 추출
        const searchResults = [];

        if (searchJsonResponse.organic_results && searchJsonResponse.organic_results.length > 0) {
            // 최대 5개 결과만 추출
            const maxResults = Math.min(5, searchJsonResponse.organic_results.length);

            for (let i = 0; i < maxResults; i++) {
                const organicResult = searchJsonResponse.organic_results[i];

                searchResults.push({
                    title: organicResult.title || "제목 없음",
                    url: organicResult.url || "",
                    description: organicResult.snippet || "설명 없음"
                });
            }
        }
        ...
        for (let i = 0; i < searchResults.length; i++) {
            ...
            formattedResult += (i + 1) + ". " + resultItem.title + "\n";
            formattedResult += "URL: " + resultItem.url + "\n";
            formattedResult += "설명: " + resultItem.description + "\n\n";
        }
...
function getJobKoreaDevData(currentDate) {
    try {
        ...
        const jobDataList = selectedLiJson.subList;
...
function getResponse(type, messages) {
    ...
    if (type === "openai") {
        ...
        return getGeminiResponse(messages[messages.length - 1].content);
    } else {
        // 기본값은 OpenAI
        url = "https://api.openai.com/v1/chat/completions";
        key = OPENAI_API_KEY;
